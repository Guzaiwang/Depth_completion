...
class CompletionFormerMetric_w_confidence(BaseMetric):
    def __init__(self, args):
        super(CompletionFormerMetric_w_confidence, self).__init__(args)

        self.args = args
        self.t_valid = 0.0001

        self.metric_name = [
            'RMSE', 'MAE', 'iRMSE', 'iMAE', 'REL', 'D^1', 'D^2', 'D^3', 'D102', 'D105', 'D110', 'Confidence', 'Conf_error', 'conf_rmse'
        ]
    ...
    def evaluate(self, sample, output, mode=None):
        with torch.no_grad():
            ...
            if 'uncertainty' in output:
                confidence_map = output['uncertainty']
                confidence_map_label = confidence_map[mask]
                confidence_ = confidence_map_label.mean()

                conf_rmse = (torch.pow(diff, 2) * confidence_map_label).sum() / (num_valid + 1e-8)

                Conf_error = (confidence_map_label * diff_abs).mean()
                print(Conf_error)

            result = [rmse, mae, irmse, imae, rel, del_1, del_2, del_3, del_102, del_105, del_110, confidence_, Conf_error, conf_rmse]
            result = torch.stack(result)
            result = torch.unsqueeze(result, dim=0).detach()
